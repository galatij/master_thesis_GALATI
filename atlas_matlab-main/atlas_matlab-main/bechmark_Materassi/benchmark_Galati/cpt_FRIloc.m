        ...
        for i2 = 1 : ngauss
            ...
            if (masksP.tstick(i,gp_idx) == true ...
                ...
                if (masksP.tstick(i,gp_idx) == true || masksP.t0(i,gp_idx) == true)
                    REStop = REStop + mode*1/gamma*Ptalpha'*Pt*wJ*dsol(top_dof) + FRI21*dsol(bot_dof); % top
                    RESbot = RESbot + mode*1/gamma*Ptalpha'*(-gamma*Nloc_bot.*tT)*wJ*dsol(top_dof) + FRI22*dsol(bot_dof); % bottom
                end

            elseif (masksP.tslide(i,gp_idx) == true ...
                    || masksP.n0(i,gp_idx) == true ...
                ...
                if (masksP.tslide(i,gp_idx) == true || masksP.t0(i,gp_idx) == true)
                    REStop = REStop + mode*phi/gamma * test_top' * trial_top1 * wJ*dsol(top_dof) + FRI1_21*dsol(bot_dof); % top
                    RESbot = RESbot + mode*phi/gamma * test_top' * trial_top1 * wJ*dsol(top_dof) + FRI1_22*dsol(bot_dof); % bottom
                end
