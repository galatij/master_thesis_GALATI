...
bound(1).isbound = (bar_fac(:,1) == xmax);
bound(1).ID = ID(bound(1).isbound);
bound(1).nf = length(bound(1).ID);
bound(1).values = zeros(bound(1).nf,3) + [-1,0,0]*100;
bound(2).isbound = (bar_fac(:,1) == xmin);
bound(2).ID = ID(bound(2).isbound);
bound(2).nf = length(bound(2).ID);
bound(2).values = zeros(bound(2).nf,3) + [1,0,0]*100;
...
loads(indU(3:3:3*nn),:) = repmat(loadsScal',nn,1);
loads(indL,:) = 1.0;
